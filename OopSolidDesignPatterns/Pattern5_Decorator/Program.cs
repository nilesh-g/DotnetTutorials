...
// 3. Observer Design Pattern using Interface
...
abstract class OrderProcessor
{
    ...
    {
        IBill bill = new BaseBill(itemSelector, priceCalculator);
        bill = new DiscountDecorator(bill, discountLogic);
        bill = new TaxDecorator(bill);
...
class DeliveryOrderService : OrderProcessor
{
    ...
    public override double CalculateBill()
    {
        IBill bill = new BaseBill(itemSelector, priceCalculator);
        bill = new DiscountDecorator(bill, discountLogic);
        bill = new PackingChargesDecorator(bill);
        bill = new TaxDecorator(bill);
        return bill.GetAmount();
    }
