...
class WellknownPaymentGateway : IPaymentGateway
{
    public void MakePayment(double amount)
    {
        // make payment
        Console.WriteLine($"Payment of Rs. {amount} made using Wellknown PaymentGateway.");
    }
}
...
class GatewayPaymentProcessor : IPaymentProcessor
{
    private IPaymentGateway gateway = new WellknownPaymentGateway();
...
class PaymentProcessorProvider
{
    public static IPaymentProcessor Create(string mode)
    {
        if (mode == "CARD")
            ...
            return new GatewayPaymentProcessor();
        return null;
    }
}
